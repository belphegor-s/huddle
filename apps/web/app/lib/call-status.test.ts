...
describe('callStatus', () => {
  it('says so while it is still joining', () => {
    expect(callStatus({ status: 'joining', peers: [], relay: false, error: null }, 1)).toBe(
      'Joining',
    );
  });
  ...
  it('counts the room when everybody is through', () => {
    expect(
      callStatus({ status: 'live', peers: [peer('connected')], relay: false, error: null }, 2),
    ).toBe('2 people');
    expect(callStatus({ status: 'live', peers: [], relay: false, error: null }, 1)).toBe(
      '1 person',
    );
  });
  ...
  it('says nothing new while a connection is still trying', () => {
    expect(
      callStatus({ status: 'live', peers: [peer('connecting')], relay: false, error: null }, 2),
    ).toBe('2 people');
  });
  ...
  it('names the deployment gap when nothing can be reached and there is no relay', () => {
    ...
    const said = callStatus(
      { status: 'live', peers: [peer('failed')], relay: false, error: null },
      2,
    );
  ...
  it('blames the connection rather than the deployment when there is a relay', () => {
    const said = callStatus(
      { status: 'live', peers: [peer('failed')], relay: true, error: null },
      2,
    );
  ...
  it('puts something the caller asked for and did not get above the count', () => {
    const said = callStatus(
      {
        status: 'live',
        peers: [],
        relay: true,
        error: 'huddle could not start sharing your screen.',
      },
      2,
    );

    expect(said).toContain('could not start sharing');
  });
});
