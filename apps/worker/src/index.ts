...
/**
 * Uploads and downloads are handled here rather than in the shared API,
 * because streaming a body into R2 is the one thing that genuinely differs
 * between platforms. Everything else lives in packages/api.
 */
async function handleBlob(request: Request, env: CloudflareBindings): Promise<Response | null> {
  ...
  if (request.method === 'PUT') {
    ...
    await blobs.put(key, request.body, request.headers.get('content-type') ?? 'application/octet-stream');
