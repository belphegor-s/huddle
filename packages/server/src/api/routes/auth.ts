import { RequestMagicLinkInput } from '@huddle/core';
import { requestMagicLink, signOut, verifyMagicLink } from '../../services/index.js';
import { Hono } from 'hono';
import { appUrl, clientIp, type ApiEnv } from '../env.js';
import { failure, jsonBody } from '../http.js';
import { endSession, startSession } from '../session.js';

export function authRoutes(): Hono<ApiEnv> {
  const routes = new Hono<ApiEnv>();

  routes.post('/magic-link', async (c) => {
    const input = await jsonBody(c, RequestMagicLinkInput);
    const sent = await requestMagicLink(c.var.app, {
      email: input.email,
      redirectTo: input.redirectTo,
      clientIp: clientIp(c),
      appUrl: appUrl(c),
    });

    if (!sent.ok) return failure(c, sent.error);
    return c.json({ ok: true, expiresAt: sent.value.expiresAt });
  });

  routes.post('/signout', async (c) => {
    const token = c.var.sessionToken;
    if (token) await signOut(c.var.app, token);
    endSession(c);
    return c.json({ ok: true });
  });

  return routes;
}

/**
 * Reached by tapping a link in a mail client, so it lives outside /api: it is
 * a page navigation, not a call. It answers with a redirect either way, so a
 * dead link lands on the sign in screen with an explanation rather than on a
 * bare error page.
 */
export function authCallbackRoutes(): Hono<ApiEnv> {
  const routes = new Hono<ApiEnv>();

  routes.get('/callback', async (c) => {
    const token = c.req.query('token');
    if (!token) return c.redirect('/signin?error=missing_link');

    const verified = await verifyMagicLink(c.var.app, token);
    if (!verified.ok) return c.redirect('/signin?error=link_expired');

    startSession(c, verified.value.sessionToken);
    return c.redirect(verified.value.redirectTo ?? '/');
  });

  return routes;
}
