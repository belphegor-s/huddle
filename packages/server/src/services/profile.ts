...
export async function describeMe(ctx: AppContext, user: User): Promise<Me> {
  return { user, workspaces: await listWorkspaces(ctx, user.id) };
}
...
export async function updateProfile(
  ...
  if (Object.keys(patch).length === 0) {
    const current = await ctx.db.select().from(users).where(eq(users.id, input.userId)).limit(1);
