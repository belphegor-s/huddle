...
export type AuthDeps = Pick<Ports, 'db' | 'kv' | 'mailer' | 'clock'>;
...
export async function requestMagicLink(
  ...
  const perEmail = await deps.kv.increment(`rl:magic:email:${input.email}`, HOUR_SECONDS);
  if (perEmail > RATE_LIMITS.magicLinkPerHourPerEmail) return err('rate_limited');

  const perIp = await deps.kv.increment(`rl:magic:ip:${input.clientIp}`, HOUR_SECONDS);
  ...
  await deps.kv.set(magicKey(await hashToken(token)), JSON.stringify(payload), {
...
export async function verifyMagicLink(
  ...
  const user = await findOrCreateUser(deps, parsed.data.email);
  const sessionToken = await createSession(deps, user.id);

  return ok({ sessionToken, user, redirectTo: parsed.data.redirectTo });
}

export async function createSession(deps: AuthDeps, userId: string): Promise<string> {
  const now = deps.clock.now();
  const token = randomToken();
  const payload: StoredSession = { userId, createdAt: now, lastSeenAt: now };

  await deps.kv.set(sessionKey(await hashToken(token)), JSON.stringify(payload), {
...
export async function loadSession(deps: AuthDeps, token: string): Promise<User | null> {
  ...
  const rows = await deps.db.select().from(users).where(eq(users.id, parsed.data.userId)).limit(1);
  const user = rows[0];
  if (!user) {
    await deps.kv.delete(key);
    return null;
  ...
  if (now - parsed.data.lastSeenAt > SESSION_REFRESH_AFTER_MS) {
    ...
    await deps.kv.set(key, JSON.stringify(refreshed), { ttlSeconds: SESSION_TTL_SECONDS });
  }
...
export async function signOut(deps: AuthDeps, token: string): Promise<void> {
  await deps.kv.delete(sessionKey(await hashToken(token)));
}

async function findOrCreateUser(deps: AuthDeps, email: string): Promise<User> {
  ...
  const inserted = await deps.db.insert(users).values(user).onConflictDoNothing().returning();
  if (inserted[0]) return inserted[0];

  const raced = await deps.db.select().from(users).where(eq(users.email, email)).limit(1);
  const found = raced[0];
