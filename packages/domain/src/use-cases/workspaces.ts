...
export type WorkspaceDeps = Pick<Ports, 'db' | 'clock'>;
...
export async function findWorkspaceBySlug(
  ...
  const member = await requireMember(deps.db, { workspaceId: workspace.id, userId: input.userId });
...
export async function createInvite(
  ...
  const member = await requireMember(deps.db, {
...
export async function describeInvite(
  ...
  const usable = await loadUsableInvite(deps, token);
...
export async function acceptInvite(
  ...
  const usable = await loadUsableInvite(deps, input.token);
  if (!usable.ok) return err(usable.error);
  ...
  const existing = await requireMember(deps.db, {
...
async function loadUsableInvite(
  ...
  if (invite.expiresAt <= deps.clock.now()) return err('expired');
