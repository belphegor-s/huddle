...
export function workspaceRoutes(): Hono<ApiEnv> {
  ...
  routes.get('/me', async (c) => c.json(await describeMe(c.var.ports, currentUser(c))));

  routes.patch('/me', async (c) => {
    const user = currentUser(c);
    const patch = await jsonBody(c, UpdateProfileInput);
    const updated = await updateProfile(c.var.ports, { userId: user.id, patch });
  ...
  routes.get('/workspaces', async (c) =>
    c.json(await listWorkspaces(c.var.ports, currentUser(c).id)),
  );

  routes.post('/workspaces', async (c) => {
    const user = currentUser(c);
    const input = await jsonBody(c, CreateWorkspaceInput);
    const created = await createWorkspace(c.var.ports, { userId: user.id, ...input });
  ...
  routes.get('/workspaces/:slug', async (c) => {
    const found = await findWorkspaceBySlug(c.var.ports, {
  ...
  routes.post('/workspaces/:workspaceId/invites', async (c) => {
    ...
    const invite = await createInvite(c.var.ports, {
  ...
  routes.delete('/workspaces/:workspaceId/invites/:inviteId', async (c) => {
    const revoked = await revokeInvite(c.var.ports, {
  ...
  routes.get('/invites/:token', async (c) => {
    const described = await describeInvite(c.var.ports, c.req.param('token'));
  ...
  routes.post('/invites/:token/accept', async (c) => {
    const joined = await acceptInvite(c.var.ports, {
